...
class AuthFilter extends CFilter
{
	...
	protected function preFilter($filterChain)
	{
		...
		// Create a task in development
		$task = $itemName . '/*';
		if (YII_DEBUG) Yii::app()->authManager->createTask($task);
		if ($user->checkAccess($task)) return true;

		$operation = $itemName . '/' . $controller->action->getId();
		if (YII_DEBUG) Yii::app()->authManager->createOperation($task);
		if ($user->checkAccess($operation, $this->params)) return true;
